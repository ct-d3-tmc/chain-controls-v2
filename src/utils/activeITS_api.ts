...
export function create_session() {
  ...
  session.connect(
    config.ACTIVEITS_API_PORT,
    config.ACTIVEITS_API_HOSTNAME,
    () => {}
  );
...
export function login_to_subsystem(
  session: any,
  subsystem: String,
  username: String
) {}
export function get_queue_msgs(
  session: any,
  token: any,
  goldeneye_username: String
) {}
export function generate_apply_command() {}
export function generate_remove_command(
  security_token: any,
  cms_id: any,
  msg_id: any,
  username: any
...
export function parse_msg_id_list(
  cmsId: number,
  xmlData: Uint8Array
): string[] {
  return [];
}
export function is_travel_time_message(
  id_num: any,
  xml_data: any,
  message: any
) {
